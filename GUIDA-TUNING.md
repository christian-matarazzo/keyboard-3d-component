...
4. Una luce direzionale non salva una faccia il cui normale guarda altrove: per riempire "a prescindere dall'orientamento" non aggiungere un frontale (spiana) — alza la `base diffusa` o affidati alla luce `Luci · orbitale (sotto)`, che orbita col modello (vedi sopra).
...
6. **Case che brucia nelle viste laterali/top**: la scocca in alluminio prende una spazzolata speculare bianca quando la `key principale (alto-sx)` e le strip la colpiscono di taglio. Il fix NON è alzare l'esposizione o l'ambiente: è tenere la key moderata (~16) e le strip contenute (top ~6, destra ~5, basso-dx ~3.5, sinistra ~1.5), lasciando che la `base diffusa` (~0.55) sollevi i neri. Il `rake laterale` illumina i soli keycaps (layer dedicato), quindi non brucia il case. Verificare SEMPRE la vista laterale pura (yaw 90°, pitch 0°) e la Top (90°, 0°): sono quelle dove il case brucia per primo.
